...
Metadata Variables are passed to your Voice Agent via [Automations](/build/automations/overview) or by using our [API](/developers). Any data that you pass via Metadata will be available as a Variable in your Voice Agent and can be accessed using `{{metadata.variable_name}}`.
