...
The `companyId` identifies the customer/company container that owns the work. It is not the legal entity being incorporated. The `incorporation.id` identifies the incorporation for that customer. Participants, documents, intake, status, and issues all attach to that incorporation.
...
The Incorporation API does not duplicate reusable customer records. People, business entities, locations, and files live in the Commenda OS Partner API. The Incorporation API links those records to a specific incorporation.
