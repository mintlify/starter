...
Partner-led incorporations are built from a few related records:
...
The `companyId` identifies the customer/company container that owns the work. It is not the legal entity being incorporated. The `incorporation.id` identifies the incorporation case for that customer. Participants, documents, intake, status, and issues all attach to that incorporation case.
...
The Incorporation API does not duplicate reusable customer records. People, business entities, locations, and files live in the Commenda OS Partner API. The Incorporation API links those records into a specific incorporation case.
