...
The `customerId` identifies the customer/company container that owns the work. It is not the legal entity being incorporated. The `incorporation.id` identifies the incorporation for that customer. Participants, documents, intake, status, and issues all attach to that incorporation.

For the full object map, see [Core concepts](/engine/incorporation/partner-incorporation-core-concepts).

## Flow at a glance

1. Create or find the customer/company container and save `customerId`.
