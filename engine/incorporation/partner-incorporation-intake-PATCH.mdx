...
Each request can be partial. Include only the values you want to add or change in `requirements`. Commenda merges them into the current intake state, validates the incorporation, and returns the updated `intakeState` with a validation summary.
