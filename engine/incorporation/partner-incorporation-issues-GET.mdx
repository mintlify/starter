...
Issues do not change `incorporationStatus` in this phase. Continue reading the incorporation resource for lifecycle status, and read issues for correction/request-for-information state.
...
| `intake_field` | The issue applies to an intake field path, such as `companyNameOptions`. |
