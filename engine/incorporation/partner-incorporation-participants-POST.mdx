...
| Same individual director and shareholder | `individual` | `keyPerson` | Include both role objects in one `roles` array. | Shareholder role still needs `ownershipPercentage`. | One participant record can satisfy both role document requirements once its required documents are upserted. |
