...
description: "Implement a partner-owned incorporation workflow from company creation through final submit for review."
---
...
| Discovery | What can I incorporate? | Read the jurisdiction catalog and choose `country` plus `countryOptions`. |
...
| Completion | How do I hand it to Commenda? | Read validation, call the submit endpoint when `intakeState.validation.isComplete` is `true`, then track `incorporationStatus`. |
| Issues | What if Commenda needs corrections? | Read active/resolved issues, update the underlying intake/participants/documents, and optionally receive issue webhooks. |
...
| `serviceRequestId` | Incorporation response | Support/debugging only | Temporary bridge field. Use `incorporation.id` in API calls. |
...
| `participantId` | Register participant | Link participant documents | Incorporation-scoped participant id. |
...
Save `incorporation.id`. The initial `incorporationStatus` is usually `collecting_information`.
...
The submit endpoint refreshes validation before accepting the handoff. If the incorporation is incomplete, it returns `400` with validation details. If it succeeds, the response returns `incorporationStatus: "ready_for_review"`.
...
| `required.when` / `visibility` | Rules that make a field required or visible only when another field has a value | Re-evaluate the UI when the controlling field changes. |
...
| --- | --- | --- |
| `companyNameOptions` | Company name options are missing or invalid | Submit valid names through the intake endpoint. |
| `providedRegisteredAddress` | A provided registered address is required | Submit a Commenda OS location reference or choose `standardAddress`, which means Commenda should use its standard registered address service. |
...
collecting_information
  -> ready_for_review   after POST /partner/incorporation/{incorporationId}/submit succeeds
  -> in_progress        when Commenda starts processing
  -> completed          when the incorporation is complete
```

Before Commenda starts processing, if validation later becomes incomplete, reads return `collecting_information` again. Rejection/cancellation flows are not part of the MVP surface; correction requests are represented by read-only issues.
...
| `400` when registering participant | Shareholder role is missing `ownershipPercentage`, or role/resource type is unsupported | Follow the selected requirements DSL. |
