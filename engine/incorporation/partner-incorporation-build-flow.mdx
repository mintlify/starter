...
| Documents | How do I attach files? | Follow [Upload documents for incorporation](/engine/incorporation/partner-incorporation-document-upload-flow): upload customer documents with multipart form data to obtain `fileId`, then upsert each file as a typed participant document. |
...
| `participantId` | Register participant | Update participant roles; upsert participant documents | Incorporation-scoped participant id. |
| `fileId` | `POST /partner/commenda-os/customers/{customerId}/documents` | Upsert participant document | Customer-scoped file id returned after multipart upload. |
...
Upload each source document as multipart form data to create a reusable customer file. Upserting the participant document scopes the file to the incorporation and assigns it to the participant document slot in one request.

<Info>
The multipart upload endpoint lives in the Commenda OS Partner API because files are reusable customer resources. For a shorter document-only walkthrough, see [Upload documents for incorporation](/engine/incorporation/partner-incorporation-document-upload-flow).
</Info>
...
curl --request POST \
  ...
  --form 'personId=12' \
  --form 'documentName=Utility bill'
```
...
| `400` when uploading customer document | Missing file part, unsupported MIME type, file too large, or both `personId` and `businessEntityId` were provided | Send exactly one binary `file` part, valid optional metadata, and a supported file type. |
