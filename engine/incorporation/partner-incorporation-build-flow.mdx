...
6. Submit incorporation-level intake fields, then read validation until `intakeState.validation.isComplete` is `true`.
...
| Submit for review | How do I hand it to Commenda? | Read validation, call the submit endpoint when `intakeState.validation.isComplete` is `true`, then track `GET /partner/incorporation/{incorporationId}/status`. |
...
| `statusWorkflow.stages[].id` | Status workflow or case status | Progress UI and issue-stage correlation | Stage ids come from the active workflow definition. |
...
Intake updates are merge-based and may be partial. Use this endpoint for non-participant data such as company name options and registered office address choice.
...
| `registeredOfficeAddress.registeredOfficeAddressLocation.resourceId` | The referenced registered office address location is missing or inaccessible | Use a Commenda OS location for the same customer record as the incorporation, or update/create the location before submitting the intake reference. |
...
`intakeState.validation.isComplete` means the current data satisfies the incorporation requirements and is eligible to submit. It does not by itself hand the incorporation to Commenda.
...
Before Commenda review work starts, if validation later becomes incomplete, reads return `awaiting_customer_input` again. Correction requests are represented by read-only issues and surface as `blocked` while active.
