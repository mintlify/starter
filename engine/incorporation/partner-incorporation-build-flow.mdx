...
| `fields` | Incorporation-level data, such as company names or registered address choice | Submit these keys under `requirements` in the intake endpoint. |
...
| `registeredAddress.providedRegisteredAddress.resourceId` | The referenced registered address location is missing or inaccessible | Use a Commenda OS location for the same customer/company record as the incorporation, or update/create the location before submitting the intake reference. |
