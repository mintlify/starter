...
This guide shows the recommended implementation sequence for a partner-owned incorporation flow. Use [Core concepts](/engine/incorporation/partner-incorporation-core-concepts) for the object model and the endpoint reference pages for exact request and response schemas.
...
3. Create the incorporation case and save `incorporation.id`.
...
| --- | --- | --- |
| `schemaVersion` / `dslVersion` | Server-side requirements version identifiers | Useful for logs and debugging. Clients do not send a requirements version back. |
