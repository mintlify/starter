...
Use this endpoint after the customer chooses `country` and `countryOptions` if you want to render a progress UI before the incorporation is created. Commenda adds an `intake` stage first so your progress UI can show data collection before review.
