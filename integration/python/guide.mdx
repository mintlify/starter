...
<div className="not-prose" style={{display: "flex", gap: "8px", padding: "0"}}>
  ...
  <a href="https://pypi.org/project/langwatch/" target="_blank">
    <img src="https://img.shields.io/pypi/v/langwatch?color=007EC6" noZoom alt="LangWatch TypeScript SDK version" />
  </a>
  </div>
</div>
...
@langwatch.trace()
def main():
    langwatch.get_current_trace().update(user_id="user_id", thread_id="thread_id")
    ...
```
...
@langwatch.trace()
def main():
    langwatch.get_current_trace().update(labels="production")
    ...
```
