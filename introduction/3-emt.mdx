...
Sage introduces the **Earned Mint Token (EMT)** model to provide a new approach to token issuance. Unlike traditional tokens, EMTs are minted solely from measurable, onchain user actions. This ensures the token supply grows exclusively from authentic, verified user contributions—eliminating arbitrary allocations and pre-mined, speculative distributions. Each EMT reward is calculated based on the transaction fee associated with the rewarded action, aligning incentives closely with user effort, naturally deterring spam, and making rewards immediately available for users to claim without centralized or subjective decision making.
