...
- Fixed an error that led to problems with updating dictionaries with the ODBC source. (https://github.com/ClickHouse/ClickHouse/issues/3825), (https://github.com/ClickHouse/ClickHouse/issues/3829)
- JIT compilation of aggregate functions now works with LowCardinality columns. (https://github.com/ClickHouse/ClickHouse/issues/3838)

#### Improvements: 

- Added the `low_cardinality_allow_in_native_format` setting (enabled by default). When disabled, LowCardinality columns will be converted to ordinary columns for SELECT queries and ordinary columns will be expected for INSERT queries. (https://github.com/ClickHouse/ClickHouse/pull/3879)
...
- `DEFAULT` expressions are evaluated for missing fields when loading data in semi-structured input formats (`JSONEachRow`, `TSKV`). The feature is enabled with the `insert_sample_with_metadata` setting. (https://github.com/ClickHouse/ClickHouse/pull/3555)
- The `ALTER TABLE` query now has the `MODIFY ORDER BY` action for changing the sorting key when adding or removing a table column. This is useful for tables in the `MergeTree` family that perform additional tasks when merging based on this sorting key, such as `SummingMergeTree`, `AggregatingMergeTree`, and so on. (https://github.com/ClickHouse/ClickHouse/pull/3581) (https://github.com/ClickHouse/ClickHouse/pull/3755)
- For tables in the `MergeTree` family, now you can specify a different sorting key (`ORDER BY`) and index (`PRIMARY KEY`). The sorting key can be longer than the index. (https://github.com/ClickHouse/ClickHouse/pull/3581)
...
- Now you can use a parameter to configure the precision of the `uniqCombined` aggregate function (select the number of HyperLogLog cells). (https://github.com/ClickHouse/ClickHouse/pull/3406)
- Added the `system.contributors` table that contains the names of everyone who made commits in ClickHouse. (https://github.com/ClickHouse/ClickHouse/pull/3452)
- Added the ability to omit the partition for the `ALTER TABLE ... FREEZE` query in order to back up all partitions at once. (https://github.com/ClickHouse/ClickHouse/pull/3514)
...
- Now you can specify comments for a column in the table description and change it using `ALTER`. (https://github.com/ClickHouse/ClickHouse/pull/3377)
...
- Added the `partition_key`, `sorting_key`, `primary_key`, and `sampling_key` columns to the `system.tables` table in order to provide information about table keys. (https://github.com/ClickHouse/ClickHouse/pull/3609)
- Added the `is_in_partition_key`, `is_in_sorting_key`, `is_in_primary_key`, and `is_in_sampling_key` columns to the `system.columns` table. (https://github.com/ClickHouse/ClickHouse/pull/3609)
...
- Fixes and performance improvements for the `LowCardinality` data type. `GROUP BY` using `LowCardinality(Nullable(...))`. Getting the values of `extremes`. Processing high-order functions. `LEFT ARRAY JOIN`. Distributed `GROUP BY`. Functions that return `Array`. Execution of `ORDER BY`. Writing to `Distributed` tables (nicelulu). Backward compatibility for `INSERT` queries from old clients that implement the `Native` protocol. Support for `LowCardinality` for `JOIN`. Improved performance when working in a single stream. (https://github.com/ClickHouse/ClickHouse/pull/3823) (https://github.com/ClickHouse/ClickHouse/pull/3803) (https://github.com/ClickHouse/ClickHouse/pull/3799) (https://github.com/ClickHouse/ClickHouse/pull/3769) (https://github.com/ClickHouse/ClickHouse/pull/3744) (https://github.com/ClickHouse/ClickHouse/pull/3681) (https://github.com/ClickHouse/ClickHouse/pull/3651) (https://github.com/ClickHouse/ClickHouse/pull/3649) (https://github.com/ClickHouse/ClickHouse/pull/3641) (https://github.com/ClickHouse/ClickHouse/pull/3632) (https://github.com/ClickHouse/ClickHouse/pull/3568) (https://github.com/ClickHouse/ClickHouse/pull/3523) (https://github.com/ClickHouse/ClickHouse/pull/3518)
- Fixed how the `select_sequential_consistency` option works. Previously, when this setting was enabled, an incomplete result was sometimes returned after beginning to write to a new partition. (https://github.com/ClickHouse/ClickHouse/pull/2863)
- Databases are correctly specified when executing DDL `ON CLUSTER` queries and `ALTER UPDATE/DELETE`. (https://github.com/ClickHouse/ClickHouse/pull/3772) (https://github.com/ClickHouse/ClickHouse/pull/3460)
- Databases are correctly specified for subqueries inside a VIEW. (https://github.com/ClickHouse/ClickHouse/pull/3521)
...
- Fixed a race condition when simultaneously reading from a `MATERIALIZED VIEW` and deleting a `MATERIALIZED VIEW` due to not locking the internal `MATERIALIZED VIEW`. (https://github.com/ClickHouse/ClickHouse/pull/3404) (https://github.com/ClickHouse/ClickHouse/pull/3694)
- Fixed the error `Lock handler cannot be nullptr.` (https://github.com/ClickHouse/ClickHouse/pull/3689)
- Fixed query processing when the `compile_expressions` option is enabled (it's enabled by default). Nondeterministic constant expressions like the `now` function are no longer unfolded. (https://github.com/ClickHouse/ClickHouse/pull/3457)
- Fixed a crash when specifying a non-constant scale argument in `toDecimal32/64/128` functions.
- Fixed an error when trying to insert an array with `NULL` elements in the `Values` format into a column of type `Array` without `Nullable` (if `input_format_values_interpret_expressions` = 1). (https://github.com/ClickHouse/ClickHouse/pull/3487) (https://github.com/ClickHouse/ClickHouse/pull/3503)
...
- Fixed the return type for `quantile*` functions from `Date` and `DateTime` types of arguments. (https://github.com/ClickHouse/ClickHouse/pull/3580)
- Fixed the `WITH` clause if it specifies a simple alias without expressions. (https://github.com/ClickHouse/ClickHouse/pull/3570)
...
- Fixed buffer overflow in the `lowerUTF8` and `upperUTF8` functions. Removed the ability to execute these functions over `FixedString` type arguments. (https://github.com/ClickHouse/ClickHouse/pull/3662)
- Fixed a rare race condition when deleting `MergeTree` tables. (https://github.com/ClickHouse/ClickHouse/pull/3680)
- Fixed a race condition when reading from `Buffer` tables and simultaneously performing `ALTER` or `DROP` on the target tables. (https://github.com/ClickHouse/ClickHouse/pull/3719)
- Fixed a segfault if the `max_temporary_non_const_columns` limit was exceeded. (https://github.com/ClickHouse/ClickHouse/pull/3788)

#### Improvements: 

- The server does not write the processed configuration files to the `/etc/clickhouse-server/` directory. Instead, it saves them in the `preprocessed_configs` directory inside `path`. This means that the `/etc/clickhouse-server/` directory does not have write access for the `clickhouse` user, which improves security. (https://github.com/ClickHouse/ClickHouse/pull/2443)
- The `min_merge_bytes_to_use_direct_io` option is set to 10 GiB by default. A merge that forms large parts of tables from the MergeTree family will be performed in `O_DIRECT` mode, which prevents excessive page cache eviction. (https://github.com/ClickHouse/ClickHouse/pull/3504)
- Accelerated server start when there is a very large number of tables. (https://github.com/ClickHouse/ClickHouse/pull/3398)
- Added a connection pool and HTTP `Keep-Alive` for connections between replicas. (https://github.com/ClickHouse/ClickHouse/pull/3594)
...
- Removed logging to `stderr` from the `re2` library for invalid or complex regular expressions. (https://github.com/ClickHouse/ClickHouse/pull/3723)
...
- The `cityHash64`, `farmHash64`, `metroHash64`, `sipHash64`, `halfMD5`, `murmurHash2_32`, `murmurHash2_64`, `murmurHash3_32`, and `murmurHash3_64` functions now work for any number of arguments and for arguments in the form of tuples. (https://github.com/ClickHouse/ClickHouse/pull/3451) (https://github.com/ClickHouse/ClickHouse/pull/3519)
...
- Added the `--secure` command line parameter in `clickhouse-benchmark` and `clickhouse-performance-test` to enable TLS. (https://github.com/ClickHouse/ClickHouse/pull/3688) (https://github.com/ClickHouse/ClickHouse/pull/3690)
...
- Added the `tcp_keep_alive_timeout` option to enable keep-alive packets after inactivity for the specified time interval. (https://github.com/ClickHouse/ClickHouse/pull/3441)
- Removed unnecessary quoting of values for the partition key in the `system.parts` table if it consists of a single column. (https://github.com/ClickHouse/ClickHouse/pull/3652)
- The modulo function works for `Date` and `DateTime` data types. (https://github.com/ClickHouse/ClickHouse/pull/3385)
- Added synonyms for the `POWER`, `LN`, `LCASE`, `UCASE`, `REPLACE`, `LOCATE`, `SUBSTR`, and `MID` functions. (https://github.com/ClickHouse/ClickHouse/pull/3774) (https://github.com/ClickHouse/ClickHouse/pull/3763) Some function names are case-insensitive for compatibility with the SQL standard. Added syntactic sugar `SUBSTRING(expr FROM start FOR length)` for compatibility with SQL. (https://github.com/ClickHouse/ClickHouse/pull/3804)
- Added the ability to `mlock` memory pages corresponding to `clickhouse-server` executable code to prevent it from being forced out of memory. This feature is disabled by default. (https://github.com/ClickHouse/ClickHouse/pull/3553)
- Improved performance when reading from `O_DIRECT` (with the `min_bytes_to_use_direct_io` option enabled). (https://github.com/ClickHouse/ClickHouse/pull/3405)
...
- The `firstSignificantSubdomain` function now processes the domains `gov`, `mil`, and `edu`. [Igor Hatarist](https://github.com/ClickHouse/ClickHouse/pull/3601) Improved performance. (https://github.com/ClickHouse/ClickHouse/pull/3628)
...
- Correct return code for the clickhouse-server init script. (https://github.com/ClickHouse/ClickHouse/pull/3516)
- The `system.metrics` table now has the `VersionInteger` metric, and `system.build_options` has the added line `VERSION_INTEGER`, which contains the numeric form of the ClickHouse version, such as `18016000`. (https://github.com/ClickHouse/ClickHouse/pull/3644)
- Removed the ability to compare the `Date` type with a number to avoid potential errors like `date = 2018-12-17`, where quotes around the date are omitted by mistake. (https://github.com/ClickHouse/ClickHouse/pull/3687)
...
- Added the ability to run integration tests when only `Docker` is installed on the system. (https://github.com/ClickHouse/ClickHouse/pull/3650)
- Added the fuzz expression test in SELECT queries. (https://github.com/ClickHouse/ClickHouse/pull/3442)
- Added a stress test for commits, which performs functional tests in parallel and in random order to detect more race conditions. (https://github.com/ClickHouse/ClickHouse/pull/3438)
...
- Fixes for builds on ARM. (https://github.com/ClickHouse/ClickHouse/pull/3709)

#### Backward Incompatible Changes: 

- Removed the ability to compare the `Date` type with a number. Instead of `toDate('2018-12-18') = 17883`, you must use explicit type conversion `= toDate(17883)` (https://github.com/ClickHouse/ClickHouse/pull/3687)
...
- Fixed an error that led to problems with updating dictionaries with the ODBC source. (https://github.com/ClickHouse/ClickHouse/issues/3825), (https://github.com/ClickHouse/ClickHouse/issues/3829)
- Databases are correctly specified when executing DDL `ON CLUSTER` queries. (https://github.com/ClickHouse/ClickHouse/pull/3460)
- Fixed a segfault if the `max_temporary_non_const_columns` limit was exceeded. (https://github.com/ClickHouse/ClickHouse/pull/3788)
...
- Fixed error in `dictGet...` function for dictionaries of type `range`, if one of the arguments is constant and other is not. (https://github.com/ClickHouse/ClickHouse/pull/3751)
- Fixed error that caused messages `netlink: '...': attribute type 1 has an invalid length` to be printed in Linux kernel log, that was happening only on fresh enough versions of Linux kernel. (https://github.com/ClickHouse/ClickHouse/pull/3749)
...
- Fixed excessive memory allocation when using large value of `max_query_size` setting (a memory chunk of `max_query_size` bytes was preallocated at once). (https://github.com/ClickHouse/ClickHouse/pull/3720)

#### Build Changes: 

- Fixed build with LLVM/Clang libraries of version 7 from the OS packages (these libraries are used for runtime query compilation). (https://github.com/ClickHouse/ClickHouse/pull/3582)

### ClickHouse Release 18.14.17, 2018-11-30 

#### Bug Fixes: 

- Fixed cases when the ODBC bridge process did not terminate with the main server process. (https://github.com/ClickHouse/ClickHouse/pull/3642)
- Fixed synchronous insertion into the `Distributed` table with a columns list that differs from the column list of the remote table. (https://github.com/ClickHouse/ClickHouse/pull/3673)
- Fixed a rare race condition that can lead to a crash when dropping a MergeTree table. (https://github.com/ClickHouse/ClickHouse/pull/3643)
- Fixed a query deadlock in case when query thread creation fails with the `Resource temporarily unavailable` error. (https://github.com/ClickHouse/ClickHouse/pull/3643)
- Fixed parsing of the `ENGINE` clause when the `CREATE AS table` syntax was used and the `ENGINE` clause was specified before the `AS table` (the error resulted in ignoring the specified engine). (https://github.com/ClickHouse/ClickHouse/pull/3692)

### ClickHouse Release 18.14.15, 2018-11-21 

#### Bug Fixes: 

- The size of memory chunk was overestimated while deserializing the column of type `Array(String)` that leads to "Memory limit exceeded" errors. The issue appeared in version 18.12.13. (https://github.com/ClickHouse/ClickHouse/issues/3589)

### ClickHouse Release 18.14.14, 2018-11-20 

#### Bug Fixes: 

- Fixed `ON CLUSTER` queries when cluster configured as secure (flag `<secure>`). (https://github.com/ClickHouse/ClickHouse/pull/3599)

#### Build Changes: 

- Fixed problems (llvm-7 from system, macos) (https://github.com/ClickHouse/ClickHouse/pull/3582)

### ClickHouse Release 18.14.13, 2018-11-08 

#### Bug Fixes: 

- Fixed the `Block structure mismatch in MergingSorted stream` error. (https://github.com/ClickHouse/ClickHouse/issues/3162)
- Fixed `ON CLUSTER` queries in case when secure connections were turned on in the cluster config (the `<secure>` flag). (https://github.com/ClickHouse/ClickHouse/pull/3465)
- Fixed an error in queries that used `SAMPLE`, `PREWHERE` and alias columns. (https://github.com/ClickHouse/ClickHouse/pull/3543)
...
- Fixed performance regression of queries that process long strings. (https://github.com/ClickHouse/ClickHouse/pull/3530)

#### Build Improvements: 

- Improvements for simplifying the Arcadia build. (https://github.com/ClickHouse/ClickHouse/pull/3475), (https://github.com/ClickHouse/ClickHouse/pull/3535)

### ClickHouse Release 18.14.12, 2018-11-02 

#### Bug Fixes: 

- Fixed a crash on joining two unnamed subqueries. (https://github.com/ClickHouse/ClickHouse/pull/3505)
...
- Fixed the error `Block structure mismatch in UNION stream: different number of columns` in LIMIT queries. (https://github.com/ClickHouse/ClickHouse/issues/2156)
- Fixed errors when merging data in tables containing arrays inside Nested structures. (https://github.com/ClickHouse/ClickHouse/pull/3397)
- Fixed incorrect query results if the `merge_tree_uniform_read_distribution` setting is disabled (it is enabled by default). (https://github.com/ClickHouse/ClickHouse/pull/3429)
- Fixed an error on inserts to a Distributed table in Native format. (https://github.com/ClickHouse/ClickHouse/issues/3411)

### ClickHouse Release 18.14.10, 2018-10-23 

- The `compile_expressions` setting (JIT compilation of expressions) is disabled by default. (https://github.com/ClickHouse/ClickHouse/pull/3410)
...
- The `WITH CUBE` modifier for `GROUP BY` (the alternative syntax `GROUP BY CUBE(...)` is also available). (https://github.com/ClickHouse/ClickHouse/pull/3172)
...
- Added functions for working with the ISO week number: `toISOWeek`, `toISOYear`, `toStartOfISOYear`, and `toDayOfYear`. (https://github.com/ClickHouse/ClickHouse/pull/3146)
- Now you can use `Nullable` columns for `MySQL` and `ODBC` tables. (https://github.com/ClickHouse/ClickHouse/pull/3362)
...
- Added the `SYSTEM FLUSH LOGS` query (forced log flushes to system tables such as `query_log`) (https://github.com/ClickHouse/ClickHouse/pull/3321)
- Now you can use pre-defined `database` and `table` macros when declaring `Replicated` tables. (https://github.com/ClickHouse/ClickHouse/pull/3251)
- Added the ability to read `Decimal` type values in engineering notation (indicating powers of ten). (https://github.com/ClickHouse/ClickHouse/pull/3153)

#### Experimental Features: 

- Optimization of the GROUP BY clause for `LowCardinality data types.` (https://github.com/ClickHouse/ClickHouse/pull/3138)
- Optimized calculation of expressions for `LowCardinality data types.` (https://github.com/ClickHouse/ClickHouse/pull/3200)

#### Improvements: 

- Significantly reduced memory consumption for queries with `ORDER BY` and `LIMIT`. See the `max_bytes_before_remerge_sort` setting. (https://github.com/ClickHouse/ClickHouse/pull/3205)
- In the absence of `JOIN` (`LEFT`, `INNER`, ...), `INNER JOIN` is assumed. (https://github.com/ClickHouse/ClickHouse/pull/3147)
...
- Fixed behavior for simultaneous DROP DATABASE/TABLE IF EXISTS and CREATE DATABASE/TABLE IF NOT EXISTS. Previously, a `CREATE DATABASE ... IF NOT EXISTS` query could return the error message "File ... already exists", and the `CREATE TABLE ... IF NOT EXISTS` and `DROP TABLE IF EXISTS` queries could return `Table ... is creating or attaching right now`. (https://github.com/ClickHouse/ClickHouse/pull/3101)
- LIKE and IN expressions with a constant right half are passed to the remote server when querying from MySQL or ODBC tables. (https://github.com/ClickHouse/ClickHouse/pull/3182)
- Comparisons with constant expressions in a WHERE clause are passed to the remote server when querying from MySQL and ODBC tables. Previously, only comparisons with constants were passed. (https://github.com/ClickHouse/ClickHouse/pull/3182)
...
- Improved performance for reading data in `JSONEachRow` format. (https://github.com/ClickHouse/ClickHouse/pull/3332)
- Added synonyms for the `LENGTH` and `CHARACTER_LENGTH` functions for compatibility. The `CONCAT` function is no longer case-sensitive. (https://github.com/ClickHouse/ClickHouse/pull/3306)
- Added the `TIMESTAMP` synonym for the `DateTime` type. (https://github.com/ClickHouse/ClickHouse/pull/3390)
...
- Memory consumption by a query is logged when it exceeds the next level of an integer number of gigabytes. (https://github.com/ClickHouse/ClickHouse/pull/3205)
- Added compatibility mode for the case when the client library that uses the Native protocol sends fewer columns by mistake than the server expects for the INSERT query. This scenario was possible when using the clickhouse-cpp library. Previously, this scenario caused the server to crash. (https://github.com/ClickHouse/ClickHouse/pull/3171)
- In a user-defined WHERE expression in `clickhouse-copier`, you can now use a `partition_key` alias (for additional filtering by source table partition). This is useful if the partitioning scheme changes during copying, but only changes slightly. (https://github.com/ClickHouse/ClickHouse/pull/3166)
...
- In the configuration of external dictionaries, `null_value` is interpreted as the value of the default data type. (https://github.com/ClickHouse/ClickHouse/pull/3330)
...
- Support for the `Date`, `DateTime`, `UUID`, and `Decimal` types as a key for the `sumMap` aggregate function. (https://github.com/ClickHouse/ClickHouse/pull/3281)
- Support for the `Decimal` data type in external dictionaries. (https://github.com/ClickHouse/ClickHouse/pull/3324)
- Support for the `Decimal` data type in `SummingMergeTree` tables. (https://github.com/ClickHouse/ClickHouse/pull/3348)
- Added specializations for `UUID` in `if`. (https://github.com/ClickHouse/ClickHouse/pull/3366)
- Reduced the number of `open` and `close` system calls when reading from a `MergeTree table`. (https://github.com/ClickHouse/ClickHouse/pull/3283)
...
- Fixed an issue with `Dictionary` tables for `range_hashed` dictionaries. This error occurred in version 18.12.17. (https://github.com/ClickHouse/ClickHouse/pull/1702)
- Fixed an error when loading `range_hashed` dictionaries (the message `Unsupported type Nullable (...)`). This error occurred in version 18.12.17. (https://github.com/ClickHouse/ClickHouse/pull/3362)
- Fixed errors in the `pointInPolygon` function due to the accumulation of inaccurate calculations for polygons with a large number of vertices located close to each other. (https://github.com/ClickHouse/ClickHouse/pull/3331) (https://github.com/ClickHouse/ClickHouse/pull/3341)
- If after merging data parts, the checksum for the resulting part differs from the result of the same merge in another replica, the result of the merge is deleted and the data part is downloaded from the other replica (this is the correct behavior). But after downloading the data part, it couldn't be added to the working set because of an error that the part already exists (because the data part was deleted with some delay after the merge). This led to cyclical attempts to download the same data. (https://github.com/ClickHouse/ClickHouse/pull/3194)
...
- Fixed the functionality of `CREATE TABLE ... ON CLUSTER ... AS SELECT ...` This error occurred in version 18.12.13. (https://github.com/ClickHouse/ClickHouse/pull/3247)
- Fixed unnecessary preparation of data structures for `JOIN`s on the server that initiates the query if the `JOIN` is only performed on remote servers. (https://github.com/ClickHouse/ClickHouse/pull/3340)
...
- Fixed a parsing error in CSV format with floating-point numbers if a non-default CSV separator is used, such as `;` (https://github.com/ClickHouse/ClickHouse/pull/3155)
...
- Fixed how `Merge` tables work on top of `Distributed` tables when using `PREWHERE`. (https://github.com/ClickHouse/ClickHouse/pull/3165)
- Bug fixes in the `ALTER UPDATE` query.
- Fixed bugs in the `odbc` table function that appeared in version 18.12. (https://github.com/ClickHouse/ClickHouse/pull/3197)
- Fixed the operation of aggregate functions with `StateArray` combinators. (https://github.com/ClickHouse/ClickHouse/pull/3188)
...
- Fixed output of types for operations using `Decimal` and integer arguments. (https://github.com/ClickHouse/ClickHouse/pull/3224)
...
- The `log_query_threads` setting (logging information about each thread of query execution) now takes effect only if the `log_queries` option (logging information about queries) is set to 1. Since the `log_query_threads` option is enabled by default, information about threads was previously logged even if query logging was disabled. (https://github.com/ClickHouse/ClickHouse/pull/3241)
...
- Fixed the compatibility problem when working on a cluster of version 18.12.17 servers and older servers at the same time. For distributed queries with GROUP BY keys of both fixed and non-fixed length, if there was a large amount of data to aggregate, the returned data was not always fully aggregated (two different rows contained the same aggregation keys). (https://github.com/ClickHouse/ClickHouse/pull/3254)
- Fixed handling of substitutions in `clickhouse-performance-test`, if the query contains only part of the substitutions declared in the test. (https://github.com/ClickHouse/ClickHouse/pull/3263)
- Fixed an error when using `FINAL` with `PREWHERE`. (https://github.com/ClickHouse/ClickHouse/pull/3298)
- Fixed an error when using `PREWHERE` over columns that were added during `ALTER`. (https://github.com/ClickHouse/ClickHouse/pull/3298)
- Added a check for the absence of `arrayJoin` for `DEFAULT` and `MATERIALIZED` expressions. Previously, `arrayJoin` led to an error when inserting data. (https://github.com/ClickHouse/ClickHouse/pull/3337)
- Added a check for the absence of `arrayJoin` in a `PREWHERE` clause. Previously, this led to messages like `Size ... does not match` or `Unknown compression method` when executing queries. (https://github.com/ClickHouse/ClickHouse/pull/3357)
...
- Minor corrections to `clickhouse-benchmark`: previously, client information was not sent to the server; now the number of queries executed is calculated more accurately when shutting down and for limiting the number of iterations. (https://github.com/ClickHouse/ClickHouse/pull/3351) (https://github.com/ClickHouse/ClickHouse/pull/3352)

#### Backward Incompatible Changes: 

- Removed the `allow_experimental_decimal_type` option. The `Decimal` data type is available for default use. (https://github.com/ClickHouse/ClickHouse/pull/3329)
...
- `invalidate_query` (the ability to specify a query to check whether an external dictionary needs to be updated) is implemented for the `clickhouse` source. (https://github.com/ClickHouse/ClickHouse/pull/3126)
...
- The `Decimal` type now supports `var*` and `stddev*` aggregate functions. (https://github.com/ClickHouse/ClickHouse/pull/3129)
- The `Decimal` type now supports mathematical functions (`exp`, `sin` and so on.) (https://github.com/ClickHouse/ClickHouse/pull/3129)
- The `system.part_log` table now has the `partition_id` column. (https://github.com/ClickHouse/ClickHouse/pull/3089)

#### Bug Fixes: 

- `Merge` now works correctly on `Distributed` tables. [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/3159)
- Fixed incompatibility (unnecessary dependency on the `glibc` version) that made it impossible to run ClickHouse on `Ubuntu Precise` and older versions. The incompatibility arose in version 18.12.13. (https://github.com/ClickHouse/ClickHouse/pull/3130)
...
- Added support for `ALTER UPDATE` queries. (https://github.com/ClickHouse/ClickHouse/pull/3035)
- Added the `allow_ddl` option, which restricts the user's access to DDL queries. (https://github.com/ClickHouse/ClickHouse/pull/3104)
- Added the `min_merge_bytes_to_use_direct_io` option for `MergeTree` engines, which allows you to set a threshold for the total size of the merge (when above the threshold, data part files will be handled using O_DIRECT). (https://github.com/ClickHouse/ClickHouse/pull/3117)
- The `system.merges` system table now contains the `partition_id` column. (https://github.com/ClickHouse/ClickHouse/pull/3099)

#### Improvements 

- If a data part remains unchanged during mutation, it isn't downloaded by replicas. (https://github.com/ClickHouse/ClickHouse/pull/3103)
- Autocomplete is available for names of settings when working with `clickhouse-client`. (https://github.com/ClickHouse/ClickHouse/pull/3106)

#### Bug Fixes: 

- Added a check for the sizes of arrays that are elements of `Nested` type fields when inserting. (https://github.com/ClickHouse/ClickHouse/pull/3118)
...
- Added the `DECIMAL(digits, scale)` data type (`Decimal32(scale)`, `Decimal64(scale)`, `Decimal128(scale)`). To enable it, use the setting `allow_experimental_decimal_type`. (https://github.com/ClickHouse/ClickHouse/pull/2846) (https://github.com/ClickHouse/ClickHouse/pull/2970) (https://github.com/ClickHouse/ClickHouse/pull/3008) (https://github.com/ClickHouse/ClickHouse/pull/3047)
- New `WITH ROLLUP` modifier for `GROUP BY` (alternative syntax: `GROUP BY ROLLUP(...)`). (https://github.com/ClickHouse/ClickHouse/pull/2948)
...
- Ctrl+C in clickhouse-client clears a query that was entered. (https://github.com/ClickHouse/ClickHouse/pull/2877)
- Added the `join_default_strictness` setting (values: `"`, `'any'`, `'all'`). This allows you to not specify `ANY` or `ALL` for `JOIN`. (https://github.com/ClickHouse/ClickHouse/pull/2982)
- Each line of the server log related to query processing shows the query ID. (https://github.com/ClickHouse/ClickHouse/pull/2482)
- Now you can get query execution logs in clickhouse-client (use the `send_logs_level` setting). With distributed query processing, logs are cascaded from all the servers. (https://github.com/ClickHouse/ClickHouse/pull/2482)
- The `system.query_log` and `system.processes` (`SHOW PROCESSLIST`) tables now have information about all changed settings when you run a query (the nested structure of the `Settings` data). Added the `log_query_settings` setting. (https://github.com/ClickHouse/ClickHouse/pull/2482)
- The `system.query_log` and `system.processes` tables now show information about the number of threads that are participating in query execution (see the `thread_numbers` column). (https://github.com/ClickHouse/ClickHouse/pull/2482)
- Added `ProfileEvents` counters that measure the time spent on reading and writing over the network and reading and writing to disk, the number of network errors, and the time spent waiting when network bandwidth is limited. (https://github.com/ClickHouse/ClickHouse/pull/2482)
- Added `ProfileEvents`counters that contain the system metrics from rusage (you can use them to get information about CPU usage in userspace and the kernel, page faults, and context switches), as well as taskstats metrics (use these to obtain information about I/O wait time, CPU wait time, and the amount of data read and recorded, both with and without page cache). (https://github.com/ClickHouse/ClickHouse/pull/2482)
- The `ProfileEvents` counters are applied globally and for each query, as well as for each query execution thread, which allows you to profile resource consumption by query in detail. (https://github.com/ClickHouse/ClickHouse/pull/2482)
- Added the `system.query_thread_log` table, which contains information about each query execution thread. Added the `log_query_threads` setting. (https://github.com/ClickHouse/ClickHouse/pull/2482)
- The `system.metrics` and `system.events` tables now have built-in documentation. (https://github.com/ClickHouse/ClickHouse/pull/3016)
...
- Now you can add (merge) states of aggregate functions by using the plus operator, and multiply the states of aggregate functions by a nonnegative constant. (https://github.com/ClickHouse/ClickHouse/pull/3062) (https://github.com/ClickHouse/ClickHouse/pull/3034)
- Tables in the MergeTree family now have the virtual column `_partition_id`. (https://github.com/ClickHouse/ClickHouse/pull/3089)

#### Experimental Features: 

- Added the `LowCardinality(T)` data type. This data type automatically creates a local dictionary of values and allows data processing without unpacking the dictionary. (https://github.com/ClickHouse/ClickHouse/pull/2830)
- Added a cache of JIT-compiled functions and a counter for the number of uses before compiling. To JIT compile expressions, enable the `compile_expressions` setting. (https://github.com/ClickHouse/ClickHouse/pull/2990) (https://github.com/ClickHouse/ClickHouse/pull/3077)
...
- Improved parsing performance for text formats (`CSV`, `TSV`). [Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/2977) (https://github.com/ClickHouse/ClickHouse/pull/2980)
...
- Increased performance and reduced memory consumption for queries to `system.tables` and `system.columns` when there is a very large number of tables on a single server. (https://github.com/ClickHouse/ClickHouse/pull/2953)
- Fixed a performance problem in the case of a large stream of queries that result in an error (the `_dl_addr` function is visible in `perf top`, but the server isn't using much CPU). (https://github.com/ClickHouse/ClickHouse/pull/2938)
...
- Improvements to the functionality for the `UUID` data type. (https://github.com/ClickHouse/ClickHouse/pull/3074) (https://github.com/ClickHouse/ClickHouse/pull/2985)
- The `UUID` data type is supported in The-Alchemist dictionaries. (https://github.com/ClickHouse/ClickHouse/pull/2822)
...
- For a `CASE` expression with conditions, you can now omit `ELSE`, which is equivalent to `ELSE NULL`. (https://github.com/ClickHouse/ClickHouse/pull/2920)
...
- You can specify an offset for `LIMIT n, m` as `LIMIT n OFFSET m`. (https://github.com/ClickHouse/ClickHouse/pull/2840)
- You can use the `SELECT TOP n` syntax as an alternative for `LIMIT`. (https://github.com/ClickHouse/ClickHouse/pull/2840)
...
- Duplicate columns can be used in a `USING` clause for `JOIN`. (https://github.com/ClickHouse/ClickHouse/pull/3006)
- `Pretty` formats now have a limit on column alignment by width. Use the `output_format_pretty_max_column_pad_width` setting. If a value is wider, it will still be displayed in its entirety, but the other cells in the table will not be too wide. (https://github.com/ClickHouse/ClickHouse/pull/3003)
...
- Support for `ATTACH TABLE ... ON CLUSTER` queries. (https://github.com/ClickHouse/ClickHouse/pull/3025)

#### Bug Fixes: 

- Fixed an issue with `Dictionary` tables (throws the `Size of offsets does not match size of column` or `Unknown compression method` exception). This bug appeared in version 18.10.3. (https://github.com/ClickHouse/ClickHouse/issues/2913)
- Fixed a bug when merging `CollapsingMergeTree` tables if one of the data parts is empty (these parts are formed during merge or `ALTER DELETE` if all data was deleted), and the `vertical` algorithm was used for the merge. (https://github.com/ClickHouse/ClickHouse/pull/3049)
- Fixed a race condition during `DROP` or `TRUNCATE` for `Memory` tables with a simultaneous `SELECT`, which could lead to server crashes. This bug appeared in version 1.1.54388. (https://github.com/ClickHouse/ClickHouse/pull/3038)
- Fixed the possibility of data loss when inserting in `Replicated` tables if the `Session is expired` error is returned (data loss can be detected by the `ReplicatedDataLoss` metric). This error occurred in version 1.1.54378. (https://github.com/ClickHouse/ClickHouse/pull/2939) (https://github.com/ClickHouse/ClickHouse/pull/2949) (https://github.com/ClickHouse/ClickHouse/pull/2964)
- Fixed a segfault during `JOIN ... ON`. (https://github.com/ClickHouse/ClickHouse/pull/3000)
- Fixed the error searching column names when the `WHERE` expression consists entirely of a qualified column name, such as `WHERE table.column`. (https://github.com/ClickHouse/ClickHouse/pull/2994)
- Fixed the "Not found column" error that occurred when executing distributed queries if a single column consisting of an IN expression with a subquery is requested from a remote server. (https://github.com/ClickHouse/ClickHouse/pull/3087)
- Fixed the `Block structure mismatch in UNION stream: different number of columns` error that occurred for distributed queries if one of the shards is local and the other is not, and optimization of the move to `PREWHERE` is triggered. (https://github.com/ClickHouse/ClickHouse/pull/2226) (https://github.com/ClickHouse/ClickHouse/pull/3037) (https://github.com/ClickHouse/ClickHouse/pull/3055) (https://github.com/ClickHouse/ClickHouse/pull/3065) (https://github.com/ClickHouse/ClickHouse/pull/3073) (https://github.com/ClickHouse/ClickHouse/pull/3090) (https://github.com/ClickHouse/ClickHouse/pull/3093)
- Fixed the `pointInPolygon` function for certain cases of non-convex polygons. (https://github.com/ClickHouse/ClickHouse/pull/2910)
- Fixed the incorrect result when comparing `nan` with integers. (https://github.com/ClickHouse/ClickHouse/pull/3024)
- Fixed an error in the `zlib-ng` library that could lead to segfault in rare cases. (https://github.com/ClickHouse/ClickHouse/pull/2854)
- Fixed a memory leak when inserting into a table with `AggregateFunction` columns, if the state of the aggregate function is not simple (allocates memory separately), and if a single insertion request results in multiple small blocks. (https://github.com/ClickHouse/ClickHouse/pull/3084)
- Fixed a race condition when creating and deleting the same `Buffer` or `MergeTree` table simultaneously.
- Fixed the possibility of a segfault when comparing tuples made up of certain non-trivial types, such as tuples. (https://github.com/ClickHouse/ClickHouse/pull/2989)
...
- Fixed an error in the `arrayDistinct` function for `Nullable` array elements. (https://github.com/ClickHouse/ClickHouse/pull/2845) (https://github.com/ClickHouse/ClickHouse/pull/2937)
...
- Fixed the segfault when re-initializing the ZooKeeper session. (https://github.com/ClickHouse/ClickHouse/pull/2917)
...
- Safe use of ODBC data sources. Interaction with ODBC drivers uses a separate `clickhouse-odbc-bridge` process. Errors in third-party ODBC drivers no longer cause problems with server stability or vulnerabilities. (https://github.com/ClickHouse/ClickHouse/pull/2828) (https://github.com/ClickHouse/ClickHouse/pull/2879) (https://github.com/ClickHouse/ClickHouse/pull/2886) (https://github.com/ClickHouse/ClickHouse/pull/2893) (https://github.com/ClickHouse/ClickHouse/pull/2921)
- Fixed incorrect validation of the file path in the `catBoostPool` table function. (https://github.com/ClickHouse/ClickHouse/pull/2894)
...
- When using clang to build, some warnings from `-Weverything` have been added, in addition to the regular `-Wall-Wextra -Werror`. (https://github.com/ClickHouse/ClickHouse/pull/2957)
- Debugging the build uses the `jemalloc` debug option.
- The interface of the library for interacting with ZooKeeper is declared abstract. (https://github.com/ClickHouse/ClickHouse/pull/2950)
...
- HTTPS can be used for replication. (https://github.com/ClickHouse/ClickHouse/pull/2760)
- Added the functions `murmurHash2_64`, `murmurHash3_32`, `murmurHash3_64`, and `murmurHash3_128` in addition to the existing `murmurHash2_32`. (https://github.com/ClickHouse/ClickHouse/pull/2791)
- Support for Nullable types in the ClickHouse ODBC driver (`ODBCDriver2` output format). (https://github.com/ClickHouse/ClickHouse/pull/2834)
- Support for `UUID` in the key columns.

#### Improvements: 

- Clusters can be removed without restarting the server when they are deleted from the config files. (https://github.com/ClickHouse/ClickHouse/pull/2777)
- External dictionaries can be removed without restarting the server when they are removed from config files. (https://github.com/ClickHouse/ClickHouse/pull/2779)
- Added `SETTINGS` support for the `Kafka` table engine. [Alexander Marshalov](https://github.com/ClickHouse/ClickHouse/pull/2781)
- Improvements for the `UUID` data type (not yet complete). (https://github.com/ClickHouse/ClickHouse/pull/2618)
- Support for empty parts after merges in the `SummingMergeTree`, `CollapsingMergeTree` and `VersionedCollapsingMergeTree` engines. (https://github.com/ClickHouse/ClickHouse/pull/2815)
- Old records of completed mutations are deleted (`ALTER DELETE`). (https://github.com/ClickHouse/ClickHouse/pull/2784)
...
- Added the `max_partition_size_to_drop` config option. (https://github.com/ClickHouse/ClickHouse/pull/2782)
...
- Added the `max_fetch_partition_retries_count` setting. (https://github.com/ClickHouse/ClickHouse/pull/2831)
- Added the `prefer_localhost_replica` setting for disabling the preference for a local replica and going to a local replica without inter-process interaction. (https://github.com/ClickHouse/ClickHouse/pull/2832)
...
- Fixed the logic for processing `REPLACE PARTITION` commands in the replication queue. If there are two `REPLACE` commands for the same partition, the incorrect logic could cause one of them to remain in the replication queue and not be executed. (https://github.com/ClickHouse/ClickHouse/pull/2814)
- Fixed a merge bug when all data parts were empty (parts that were formed from a merge or from `ALTER DELETE` if all data was deleted). This bug appeared in version 18.1.0. (https://github.com/ClickHouse/ClickHouse/pull/2930)
...
- Fixed incorrect behavior of materialized views containing DISTINCT. (https://github.com/ClickHouse/ClickHouse/issues/2795)
...
- The allocator has been replaced: `jemalloc` is now used instead of `tcmalloc`. In some scenarios, this increases speed up to 20%. However, there are queries that have slowed by up to 20%. Memory consumption has been reduced by approximately 10% in some scenarios, with improved stability. With highly competitive loads, CPU usage in userspace and in system shows just a slight increase. (https://github.com/ClickHouse/ClickHouse/pull/2773)
- Use of libressl from a submodule. (https://github.com/ClickHouse/ClickHouse/pull/1983) (https://github.com/ClickHouse/ClickHouse/pull/2807)
- Use of unixodbc from a submodule. (https://github.com/ClickHouse/ClickHouse/pull/2789)
- Use of mariadb-connector-c from a submodule. (https://github.com/ClickHouse/ClickHouse/pull/2785)
...
- Added support for ON expressions for the JOIN ON syntax:
    ...
    The expression must be a chain of equalities joined by the AND operator. Each side of the equality can be an arbitrary expression over the columns of one of the tables. The use of fully qualified column names is supported (`table.name`, `database.table.name`, `table_alias.name`, `subquery_alias.name`) for the right table. (https://github.com/ClickHouse/ClickHouse/pull/2742)
- HTTPS can be enabled for replication. (https://github.com/ClickHouse/ClickHouse/pull/2760)

#### Improvements: 

- The server passes the patch component of its version to the client. Data about the patch version component is in `system.processes` and `query_log`. (https://github.com/ClickHouse/ClickHouse/pull/2646)
...
- Added the hash function `murmurHash2_32` (https://github.com/ClickHouse/ClickHouse/pull/2756).

#### Improvements: 

- Now you can use the `from_env` (https://github.com/ClickHouse/ClickHouse/pull/2741) attribute to set values in config files from environment variables.
- Added case-insensitive versions of the `coalesce`, `ifNull`, and `nullIf functions` (https://github.com/ClickHouse/ClickHouse/pull/2752).

#### Bug Fixes: 

- Fixed a possible bug when starting a replica (https://github.com/ClickHouse/ClickHouse/pull/2759).
...
- Added system tables: `formats`, `data_type_families`, `aggregate_function_combinators`, `table_functions`, `table_engines`, `collations` (https://github.com/ClickHouse/ClickHouse/pull/2721).
- Added the ability to use a table function instead of a table as an argument of a `remote` or `cluster table function` (https://github.com/ClickHouse/ClickHouse/pull/2708).
- Support for `HTTP Basic` authentication in the replication protocol (https://github.com/ClickHouse/ClickHouse/pull/2727).
...
- The `ALTER TABLE t DELETE WHERE` query does not rewrite data parts that were not affected by the WHERE condition (https://github.com/ClickHouse/ClickHouse/pull/2694).
...
- Support for the `ALTER TABLE t DELETE WHERE` query for non-replicated MergeTree tables ((https://github.com/ClickHouse/ClickHouse/pull/2634)).
- Support for arbitrary types for the `uniq*` family of aggregate functions ((https://github.com/ClickHouse/ClickHouse/issues/2010)).
- Support for arbitrary types in comparison operators ((https://github.com/ClickHouse/ClickHouse/issues/2026)).
- The `users.xml` file allows setting a subnet mask in the format `10.0.0.1/255.255.255.0`. This is necessary for using masks for IPv6 networks with zeros in the middle ((https://github.com/ClickHouse/ClickHouse/pull/2637)).
- Added the `arrayDistinct` function ((https://github.com/ClickHouse/ClickHouse/pull/2670)).
...
- Fixed an error during a CAST to Nullable types ((https://github.com/ClickHouse/ClickHouse/issues/1322)).
...
- Fixed performance degradation for queries containing `IN (subquery)` expressions inside another subquery ((https://github.com/ClickHouse/ClickHouse/issues/2571)).
...
- Added missing quoting of identifiers for queries to an external DBMS ((https://github.com/ClickHouse/ClickHouse/issues/2635)).
...
- The `has` function now works correctly for an array with Nullable elements ((https://github.com/ClickHouse/ClickHouse/issues/2115)).
...
- Fixed how an empty `TinyLog` table works after inserting an empty data block ((https://github.com/ClickHouse/ClickHouse/issues/2563)).
...
- Improved performance, reduced memory consumption, and correct memory consumption tracking with use of the IN operator when a table index could be used ((https://github.com/ClickHouse/ClickHouse/pull/2584)).
...
- Added support for `Array(Tuple(...))` arguments for the `arrayEnumerateUniq` function ((https://github.com/ClickHouse/ClickHouse/pull/2573)).
- Added `Nullable` support for the `runningDifference` function ((https://github.com/ClickHouse/ClickHouse/pull/2594)).
- Improved query analysis performance when there is a very large number of expressions ((https://github.com/ClickHouse/ClickHouse/pull/2572)).
- Faster selection of data parts for merging in `ReplicatedMergeTree` tables. Faster recovery of the ZooKeeper session ((https://github.com/ClickHouse/ClickHouse/pull/2597)).
...
- Fixed a bug in the index for MergeTree tables if the primary key column is located inside the function for converting types between signed and unsigned integers of the same size ((https://github.com/ClickHouse/ClickHouse/pull/2603)).
- Fixed segfault if `macros` are used but they aren't in the config file ((https://github.com/ClickHouse/ClickHouse/pull/2570)).
- Fixed switching to the default database when reconnecting the client ((https://github.com/ClickHouse/ClickHouse/pull/2583)).
...
- Fixed an error when reading an array column from a Nested structure ((https://github.com/ClickHouse/ClickHouse/issues/2066)).
...
- Fixed an error when reading from ReplacingMergeTree with a condition in PREWHERE that filters all rows ((https://github.com/ClickHouse/ClickHouse/issues/2525)).
...
- Fixed pruning of parts for queries with conditions on partition key columns ((https://github.com/ClickHouse/ClickHouse/issues/2342)).
- Merges are now possible after `CLEAR COLUMN IN PARTITION` ((https://github.com/ClickHouse/ClickHouse/issues/2315)).
...
- Fixed exit behavior of `clickhouse-client` in multiline mode ((https://github.com/ClickHouse/ClickHouse/issues/2510)).
...
- Fixed a header file conflict in Fedora Rawhide ((https://github.com/ClickHouse/ClickHouse/issues/2520)).
