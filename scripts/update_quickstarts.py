...
def extract_quickstart_data(file_path: Path, base_dir: Path) -> Dict[str, Any]:
    ...
    quickstart = {
        ...
        'level': frontmatter.get('level', 'Beginner'),  # Default to 'Beginner' if not specified
    }
...
def generate_badges(use_cases: List[str], products: List[str], level: str = 'Beginner') -> str:
    """
    Generate Badge components for use cases, products, and level.
    ...
    # Add level badge with appropriate color
    level_color_map = {
        'Beginner': 'green',
        'Intermediate': 'yellow',
        'Advanced': 'red'
...
def update_quickstart_badges(file_path: Path, use_cases: List[str], products: List[str], level: str = 'Beginner') -> None:
    ...
    # Generate badges
    badges = generate_badges(use_cases, products, level)
...
def main():
    ...
    for file_path in files:
        try:
            data = extract_quickstart_data(file_path, docs_dir)
            quickstarts.append(data)
            print(f"  ✓ {file_path.name}: {data['title']}")

            # Update badges in the file
            update_quickstart_badges(file_path, data['useCases'], data['products'], data['level'])
