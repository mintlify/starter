Welcome to Prem AI cookbook section. In this example, we will build a simple URL Summarizer tool step by step using [LangChain Prem AI](https://python.langchain.com/v0.2/docs/integrations/chat/premai/). Prem SDK can be used with LangChain for extended developement. This example shows one of the way on how we can extend developement with Prem AI. 

To give a nice visualization, we use streamlit and here is how the final app would look like:
...
So without further adue, let's get started. You can find the full code [here](https://github.com/premAI-io/cookbook/url-summarizer).
...
Let's start by creating a virtual environment and installing some dependencies
...
Up next, we need to install some dependencies. You can checkout all the dependencies [in this requirements file](https://github.com/premAI-io/cookbook/url-summarizer/requirements.txt). You can install it from here. 
...
<Note>
Both `premai_project_id` is a dummy id. So in your case please ensure you have a valid project_id. You can know more about project_id [here](/get-started/projects.mdx).
</Note>

## Understanding Map Reduce Summarization 

In this cookbook tutorial we are going to implement Map Reduce Summarization chain from LangChain. So first it is important to understand how does this chain operates on high level. After this, we implement this using Prem AI and LangChain. 

### Map Reduce 

The original concept comes from the famous Map-Reduce programming model which is a part of [Hadoop framework from Apache](https://hadoop.apache.org/docs/current/hadoop-mapreduce-client/hadoop-mapreduce-client-core/MapReduceTutorial.html). Primarily Map Reduce is used to simplify very large dataset processing across many machines in a cluster. The model consists of two main steps:
...
You can take a deep dive to the original working [here](https://static.googleusercontent.com/media/research.google.com/en//archive/mapreduce-osdi04.pdf). 
...
Map Reduce summarization method is used for long length documents, which potentially exceeds the token limit of the language model. It does the following:

**Map step:** The Map step first chunks the documents into smaller chunks and then applies the summarization pipeline [(LLMChain)](https://api.python.langchain.com/en/latest/chains/langchain.chains.llm.LLMChain.html#langchain.chains.llm.LLMChain) into each of the chunks 

**Reduce Step:** Once all the chunks (parts of documents are summarized) we combine all those summarized chunks and then call the LLM once again to summarize that which gives us the final summarization.

## Code walkthrough

Now let's dive into the coding our summarization chain. 

### Prompt Templates

We start off by defining our prompt templates. We need to define two templates. The first one `map_template` would instruct the LLM to take all the docs (here the chunks of the documents extracted from the URL) and identify the main theme, extract useful information etc. In the second template `reduce_template`, we say to summarize the doc (which will be combined chunks summarization) to a single summary which gives some good insight to the reader. 
...
Now we write our summarization pipeline. This can be done with the following steps:
...
3. Initialize the `map_chain` and `reduce_chain` from langchain `LLMChain` which will be using a LLM (in our case it will be ChatPremAI client) and the corresponding template. 
...
6. Finally a map reduce chain which combines all the above chains into a single chain. 
7. Run the chain and return the result 
...
Phew, that was a lot, congratulations if you made till here. Now we code our frontend interface using Streamlit. 

## Writing the frontend using Streamlit

Please note: We are only going to focus on the main summarizing component of the application. In the actual code, some more styling is applied to make the app look better. 

### Define the summarization component

Let's write a simple helper function that uses this `summarize_url` function and creates a expandable card like container which contain the summary of the URL. 
...
Finally let's use the function with the main running code. We create a form which has a text-area where we dump all the urls. Our code first checks the URL. For all successfull runs, it creates a card which when expanded contains the summary and also return a json which tells which all links are not summarized / not valid (if any). 
...
We intentionally used `_` because there is no use of this returned list of summaries. However we can extend this by uploading all the summary aling with documents (collected from `passed`) to Prem Repositories. We will show that in our next tutorial. 
