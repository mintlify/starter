...
We also create a simple webapp using streamlit which looks like this:
...
/>
 ...
 You can find the [full code here](https://github.com/premAI-io/cookbook/arxiv-ml-qna).
...
Before getting started, we need to create a new virtual environment and install all our required packages from this [requirements.txt](https://github.com/premAI-io/cookbook/arxiv-ml-qna/requirements.txt) file. To install Qdrant engine, you need to have [docker](https://www.docker.com/) installed. You can build and run the Qdrant's official docker image using the following command:
...
<Accordion title="Streamlit side bar">
```python Python
with st.sidebar:
...
Finally we write our chat function. This chat function will use our RAG module to do the retrieval and augment the LLM generations with the contexts retrieved.
