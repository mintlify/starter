...
Configuration represents client-wide settings that control behavior across all users and accounts, including payslip uploads, payroll lookback, and recurring check scheduling.

**Example:** Update client configuration
```bash
curl --request PUT \
  ...
  "client_logo": "https://cdn.client.com/logo.png",
  "client_callback_url": "https://app.client.com/callback",
  "recurring_checks_enabled": true,
  "recurring_check_frequency": "MONTHLY",
  "recurring_check_end_date": "2025-01-10T00:00Z"
}'
```

Use GET `/configuration` to retrieve the current client configuration. If no configuration exists yet, the endpoint returns 404 ("Client configuration not yet configured"). Use PUT `/configuration` to create or update configuration; updates are partial—omitted fields keep their existing values. Validation rules: `recurring_check_frequency` must be one of [WEEKLY, MONTHLY, HOURLY]; `recurring_check_end_date` must be a valid ISO8601 UTC datetime. Some user-level settings (see Users API) can override these client-level defaults.
